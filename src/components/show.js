...
export default function Show()
{
    function Deletefun (email){
        axios.delete("https://jfsd-spring-hosting-production-ed8c.up.railway.app/delete",{params:{
            email:email
        }}).then((res)=>{
            alert(res.data);
            setResult(null)
    ...
    function saveEdit(){
        axios.put("https://jfsd-spring-hosting-production-ed8c.up.railway.app/update",{
    ...
    if(result==null){
        axios.get("https://jfsd-spring-hosting-production-ed8c.up.railway.app/all", {}).then((res)=>{
        setResult(res.data)
    })
}
    if(result==null){
    ...
    return (
        <div>
            ...
            {result.map((obj,index)=>{
                return(
                    <tr style={{backgroundColor: index%2 === 0?'lightgreen':'lightblue'}}>
                        <td>{obj.name}</td>
                        <td>{obj.role}</td>
                        <td>{obj.email}</td>
                        <td>{obj.password}</td>
            ...
            <div id="edit" style={{ display:"none" }}>
                ...
                <br/>
                Password:<input type="text" name="e_pass"/>
                <br/>
